...
class NodeMasker(nn.Module):
    def __init__(self, input_dim, mask_p=0.7, replace_p=0.05) -> None:
        super().__init__()
        self.mask_p = mask_p
        self.replace_p = replace_p
        self.mask_token = 0 #nn.Parameter(torch.zeros(1, input_dim))

    def forward(self, x: torch.Tensor):
        num_nodes = x.size(0)

        mask = torch.rand(num_nodes) < self.mask_p
        mask_idx = torch.arange(num_nodes)[mask]

        if self.replace_p > 0:

            chosen_replace_mask = torch.rand(mask_idx.size(0)) < self.replace_p
            replace_idx = mask_idx[chosen_replace_mask]
            token_idx = mask_idx[~chosen_replace_mask]

            out_x = x.clone()
            chosen_token = torch.randint(num_nodes, size=(replace_idx.size(0),))
            out_x[replace_idx] = out_x[chosen_token]
            out_x[mask_idx] = 0.
        ...
        out_x[token_idx] += self.mask_token
